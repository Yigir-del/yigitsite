...
import { tierDisplay } from '../../utils/leagueAssets';
...
export default function RankProgression({ progression, embedded }: RankProgressionProps) {
  ...
  return (
    <div className={`league-rank-ladder-wrap${embedded ? ' league-rank-ladder-wrap--embedded' : ''}`}>
      {!embedded && <h3 className="league-section-title">Rank Progression</h3>}
      <p className="league-section-desc league-section-desc--tight">
        Mevcut tier: {tierDisplay(currentTier, '')} — LP geçmişi olmadığı için sahte progression
        grafiği gösterilmiyor.
      </p>
        ...
        {visibleTiers.map((tier, i) => (
          <span key={tier} style={{ display: 'contents' }}>
            ...
            >
              <RankEmblem
                tier={tier}
                size={i === currentTierIndex ? 'ladder-current' : 'ladder'}
              />
